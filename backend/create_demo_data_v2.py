"""
Demo Data Generator for GZEAMS - Version 2

Creates comprehensive demo data for all business objects.
Run with: docker-compose exec backend python manage.py shell < create_demo_data_v2.py
"""
import os
import sys
import django
...
from apps.software_licenses.models import SoftwareLicense, LicenseAllocation, Software
...
from apps.insurance.models import (
    InsurancePolicy, ClaimRecord, InsuranceCompany,
    InsuredAsset, PremiumPayment, PolicyRenewal
)
from apps.leasing.models import (
    LeaseContract, LeaseItem, LeaseReturn,
    LeaseExtension, RentPayment
)
from apps.workflows.models import (
    WorkflowDefinition, WorkflowInstance,
    WorkflowTask, WorkflowTemplate, WorkflowApproval,
...
for consumable in Consumable.objects.filter(organization=org):
    stock, created = ConsumableStock.objects.get_or_create(
        ...
        defaults={
            ...
            'warehouse_location': random.choice(list(location_map.values())).path,
            'created_by': admin_user,
        }
    )
    if created:
...
for i in range(1, 6):
    ...
    if created:
        print(f"  - {plan.plan_no}: {plan.name}")
...
for i in range(1, 9):
    ...
    if created:
        print(f"  - {task.task_no}: {task.title}")
...
print("Creating Software Catalog (5)...")
software_data = [
    {'code': 'WIN11', 'name': 'Windows 11', 'vendor': 'Microsoft', 'type': 'os'},
    {'code': 'OFF365', 'name': 'Office 365', 'vendor': 'Microsoft', 'type': 'office'},
    {'code': 'PHShop', 'name': 'Photoshop', 'vendor': 'Adobe', 'type': 'professional'},
    {'code': 'VS2022', 'name': 'Visual Studio 2022', 'vendor': 'Microsoft', 'type': 'development'},
    {'code': 'SQL2022', 'name': 'SQL Server 2022', 'vendor': 'Microsoft', 'type': 'database'},
]
...
for sw_data in software_data:
    software, created = Software.objects.get_or_create(
        ...
        defaults={
            ...
            'software_type': sw_data['type'],
            'created_by': admin_user,
        }
    )
    software_map[sw_data['code']] = software
    if created:
        print(f"  - {software.code}: {software.name}")
...
for i in range(1, 11):
    ...
    software_license, created = SoftwareLicense.objects.get_or_create(
        ...
        defaults={
            'software': random.choice(list(software_map.values())),
    ...
    if created:
        print(f"  - {software_license.license_key}: {software_license.software.name}")
...
for i in range(1, 9):
    ...
    task, created = InventoryTask.objects.get_or_create(
        ...
        defaults={
            ...
            'scanned_count': random.randint(40, 200),
            'exception_count': random.randint(0, 10),
            'created_by': admin_user,
        }
    )
    if created:
        print(f"  - {task.task_code}: {task.title} ({task.status})")
...
request_statuses = ['draft', 'submitted', 'approved', 'rejected', 'processing', 'completed']
for i in range(1, 11):
...
policy_statuses = ['active', 'expired', 'terminated', 'pending']
for i in range(1, 11):
...
print("Creating Lease Contracts (6)...")
...
for i in range(1, 7):
    ...
    contract, created = LeaseContract.objects.get_or_create(
        organization=org,
        contract_no=f'ZL{i:06d}',
        defaults={
            'lessee_name': random.choice(list(dept_map.values())).name,
            'lessor_name': random.choice(['华融租赁', '国银租赁', '远东租赁', '平安租赁']),
            'asset_description': random.choice(['办公设备', '车辆', '机械设备']),
            'start_date': start_date,
            'end_date': end_date,
            'monthly_rent': random_decimal(5000, 50000),
            'deposit': random_decimal(10000, 100000),
            'total_amount': random_decimal(100000, 1000000),
            'payment_type': random.choice(['monthly', 'quarterly', 'semi_annual']),
            'status': random.choice(contract_statuses),
            'created_by': admin_user,
        }
    )
    if created:
        print(f"  - {contract.contract_no}: {contract.asset_description}")
...
for i in range(1, 13):
    ...
    instance, created = WorkflowInstance.objects.get_or_create(
        ...
        defaults={
            ...
            'variables': {'amount': str(random_decimal(1000, 50000)), 'reason': f'业务流程{i}'},
            'created_by': admin_user,
        }
    )
    if created:
...
voucher_statuses = ['draft', 'submitted', 'approved', 'posted']
for i in range(1, 11):
    ...
    voucher, created = FinanceVoucher.objects.get_or_create(
        ...
        defaults={
            ...
            'business_type': random.choice(['purchase', 'depreciation', 'other']),
...
print(f"Total Asset Categories: {AssetCategory.objects.filter(organization=org).count()}")
print(f"Total Locations: {Location.objects.filter(organization=org).count()}")
print(f"Total Suppliers: {Supplier.objects.filter(organization=org).count()}")
...
print(f"Total Consumable Stock: {ConsumableStock.objects.filter(organization=org).count()}")
print(f"Total Maintenances: {Maintenance.objects.filter(organization=org).count()}")
print(f"Total Maintenance Plans: {MaintenancePlan.objects.filter(organization=org).count()}")
print(f"Total Maintenance Tasks: {MaintenanceTask.objects.filter(organization=org).count()}")
print(f"Total Software: {Software.objects.filter(organization=org).count()}")
print(f"Total Software Licenses: {SoftwareLicense.objects.filter(organization=org).count()}")
...
print(f"Total Asset Receipts: {AssetReceipt.objects.filter(organization=org).count()}")
print(f"Total Insurance Companies: {InsuranceCompany.objects.filter(organization=org).count()}")
print(f"Total Insurance Policies: {InsurancePolicy.objects.filter(organization=org).count()}")
print(f"Total Claim Records: {ClaimRecord.objects.filter(organization=org).count()}")
print(f"Total Lease Contracts: {LeaseContract.objects.filter(organization=org).count()}")
print(f"Total Workflow Definitions: {WorkflowDefinition.objects.filter(organization=org).count()}")
print(f"Total Workflow Instances: {WorkflowInstance.objects.filter(organization=org).count()}")
print(f"Total Workflow Tasks: {WorkflowTask.objects.filter(organization=org).count()}")
print(f"Total Finance Vouchers: {FinanceVoucher.objects.filter(organization=org).count()}")
print("\nDefault Login: admin / admin123")
print("Demo Users: zhang_san, li_si, wang_wu, zhao_liu, chen_qi, zhou_ba, wu_jiu, zheng_shi")
