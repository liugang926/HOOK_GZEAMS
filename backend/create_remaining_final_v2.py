...
Run with: docker-compose exec -T backend python manage.py shell < create_remaining_final_v2.py
"""
import os
import django
...
print("\n=== MAINTENANCE PLANS (3) ===")
for i in range(1, 4):
    plan_code = f'MP2025{i:03d}'
    plan, created = MaintenancePlan.objects.get_or_create(
        organization=org, plan_code=plan_code,
        defaults={
            'plan_name': f'季度维护计划-{i}',
            'cycle_type': 'quarterly',
            'start_date': date(2025, 1, 1),
            'end_date': date(2025, 12, 31),
            'maintenance_content': f'按季度进行设备维护检查',
    ...
    if created:
        print(f"Plan: {plan.plan_code} - {plan.plan_name}")
