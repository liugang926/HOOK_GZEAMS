...
Run with: docker-compose exec -T backend python manage.py shell < create_remaining_final_v3.py
"""
import os
import django
...
plans = list(MaintenancePlan.objects.filter(organization=org))

print(f"Found {len(assets)} assets, {len(departments)} departments, {len(demo_users)} demo users, {len(plans)} plans")

print("\n=== MAINTENANCE TASKS (6) ===")
task_statuses = ['pending', 'in_progress', 'completed', 'skipped', 'overdue']
for i in range(1, 7):
    ...
    # Use first plan if available, otherwise skip task creation
    if not plans:
        print("No maintenance plans available, skipping tasks")
        break

    task, created = MaintenanceTask.objects.get_or_create(
        organization=org, task_no=task_no,
        defaults={
            'plan': plans[i % len(plans)],  # Cycle through available plans
            'asset': random.choice(assets) if assets else None,
            'status': random.choice(task_statuses),
            'scheduled_date': scheduled_date,
            'deadline_date': deadline_date,
            'executor': random.choice(demo_users),
            'execution_content': f'设备维护执行记录 #{i}',
            'start_time': scheduled_date if random.random() > 0.3 else None,
            'end_time': scheduled_date + timedelta(hours=random.randint(1, 4)) if random.random() > 0.5 else None,
            'created_by': admin_user,
        }
    )
    if created:
        print(f"Task: {task.task_no} - {task.status}")
