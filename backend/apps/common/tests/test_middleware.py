...
class TestOrganizationMiddleware:
    ...
    def test_extract_prefers_current_organization_over_primary(
        self,
        db,
        organization,
        second_organization,
        ...
        """Fallback resolution should prefer the current active organization."""
        from apps.common.middleware import OrganizationMiddleware
        from apps.accounts.models import UserOrganization
        ...
        org_id = middleware._extract_organization_id(request)
        assert org_id == str(second_organization.id)
    ...
    def test_extract_from_superuser_default_organization(self, db, organization, superuser):
        ...
        request._cached_user = superuser
        request.session = {}
        request.GET = {}
        request.auth = None
        ...
        membership = UserOrganization.objects.get(user=superuser, organization=superuser.current_organization)
        assert org_id == str(superuser.current_organization_id)
        assert membership.is_primary is True
        assert membership.role == 'admin'
        assert superuser.current_organization is not None
