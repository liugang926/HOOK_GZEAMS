...
class BaseCRUDService:
    ...
    def _is_tenant_scoped_model(self) -> bool:
        """Check whether the service model uses tenant-aware isolation."""
        default_manager = getattr(self.model_class, '_default_manager', None)
        return isinstance(default_manager, TenantManager)

    def _resolve_organization_id(self, organization_id=None, user=None):
        """Resolve the effective organization for tenant-scoped operations."""
        ...
        resolved_org_id = normalize_organization_id(organization_id)
        if resolved_org_id:
            ...
            set_current_organization(resolved_org_id)
            return resolved_org_id

        resolved_org_id = normalize_organization_id(get_current_organization())
        if resolved_org_id:
            if user and getattr(user, 'is_authenticated', False):
                if not BaseOrganizationService.check_organization_access(
                    user,
        ...
        if user and getattr(user, 'is_authenticated', False):
            ...
            if default_organization:
                resolved_org_id = normalize_organization_id(default_organization.id)
                set_current_organization(resolved_org_id)
                return resolved_org_id

        return None

    def _scope_queryset(self, queryset: QuerySet, organization_id=None, user=None) -> QuerySet:
        """Apply fail-closed organization scoping for tenant-aware models."""
        if not self._is_tenant_scoped_model():
            return queryset
    ...
    def create(self, data: Dict, user, **kwargs) -> BaseModel:
        ...
        if 'organization_id' not in data and 'organization' not in data:
            ...
            elif self._is_tenant_scoped_model():
                raise ValidationError('Organization context is required.')

        return self.model_class.objects.create(**data)

    def update(self, instance_id: str, data: Dict, user=None, organization_id=None) -> BaseModel:
        """
        ...
        """
        instance = self.get(instance_id, organization_id=organization_id, user=user)
    ...
    def delete(self, instance_id: str, user=None, organization_id=None) -> bool:
        """
    ...
    def restore(self, instance_id: str, user=None, organization_id=None) -> BaseModel:
        """
    ...
    def get(
        ...
        if allow_deleted:
            queryset = self.model_class.all_objects.all()
        else:
            queryset = self.model_class.all_objects.filter(is_deleted=False)
