...
class WorkflowDefinitionViewSet(BaseModelViewSetWithBatch):
    ...
    @action(detail=True, methods=['get', 'put'], url_path='form-permissions')
    def form_permissions(self, request, pk=None):
        """
        Get or update form field permissions for a workflow definition.

        GET  /api/workflows/definitions/{id}/form-permissions/
             Returns all per-node form permissions.

        PUT  /api/workflows/definitions/{id}/form-permissions/
             Body: { "node_id": { "field_code": "editable|read_only|hidden", ... }, ... }
             Replaces the entire form_permissions mapping.
        """
        workflow = self.get_object()
        ...
        if request.method == 'GET':
            ...
            # Also return available fields for the associated business object
            available_fields = perm_service.get_available_fields(
                workflow.business_object_code
            )
            ...
            return Response({
                ...
                'data': {
                    ...
                    'nodes': [
                        {'id': n.get('id'), 'name': n.get('text', n.get('id')), 'type': n.get('type')}
                        for n in workflow.graph_data.get('nodes', [])
                        if n.get('type') in ('approval', 'start')
                    ],
                }
            })

        # PUT — replace entire form_permissions
        new_permissions = request.data
        if not isinstance(new_permissions, dict):
            return Response({
                'success': False,
                'error': {
                    'code': 'VALIDATION_ERROR',
                    'message': _('form_permissions must be a JSON object mapping node_id → {field_code: permission_level}.')
                }
            }, status=status.HTTP_400_BAD_REQUEST)

        # Validate permission levels
        valid_levels = {'editable', 'read_only', 'hidden'}
        for node_id, fields in new_permissions.items():
            if not isinstance(fields, dict):
                return Response({
                    'success': False,
                    'error': {
                        'code': 'VALIDATION_ERROR',
                        'message': _(f'Permissions for node "{node_id}" must be a dict.')
                    }
                }, status=status.HTTP_400_BAD_REQUEST)
            for field_code, perm_level in fields.items():
                if perm_level not in valid_levels:
                    return Response({
                        'success': False,
                        'error': {
                            'code': 'VALIDATION_ERROR',
                            'message': _(f'Invalid permission level "{perm_level}" for field "{field_code}". '
                                         f'Allowed: editable, read_only, hidden.')
                        }
                    }, status=status.HTTP_400_BAD_REQUEST)

        workflow.form_permissions = new_permissions
        workflow.save(update_fields=['form_permissions', 'updated_at'])

        return Response({
            'success': True,
            'message': _('Form permissions updated successfully.'),
            'data': {
                'form_permissions': workflow.form_permissions,
            }
        })
