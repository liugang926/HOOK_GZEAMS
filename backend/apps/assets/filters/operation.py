...
class AssetReturnFilter(BaseModelFilter):
    ...
    project = django_filters.UUIDFilter(
        ...
        help_text='Filter by linked asset project ID'
    )
    ...
    def filter_project(self, queryset, name, value):
        """Filter returns linked to allocations under the given project."""
        if not value:
            return queryset
        return queryset.filter(
            items__project_allocation__project_id=value
        ).distinct()

    class Meta:
        model = AssetReturn
        fields = [
            'return_no', 'status', 'returner', 'return_location', 'project',
