...
class AssetViewSet(BaseModelViewSetWithBatch):
    ...
    @action(detail=True, methods=['get'], url_path='lifecycle-timeline')
    def lifecycle_timeline(self, request, pk=None):
        """
        Get closed-loop lifecycle timeline for an asset.

        GET /api/assets/{id}/lifecycle-timeline/
        """
        asset = self.get_object()
