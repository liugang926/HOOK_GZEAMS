...
class Command(BaseCommand):
    help = 'Create sample test data for Assets, Categories, Locations, and Suppliers'
    ...
    def handle(self, *args, **options):
        ...
        for cat_data in categories_data:
            ...
            if created:
                self.stdout.write(f"  ✅ Created category: {cat.code} - {cat.name}")
            else:
                self.stdout.write(f"  ℹ️  Category already exists: {cat.code} - {cat.name}")
        ...
        for loc_data in locations_data:
            ...
            if created:
                self.stdout.write(f"  ✅ Created location: {location.path}")
            else:
                self.stdout.write(f"  ℹ️  Location already exists: {location.path}")
        ...
        for sup_data in suppliers_data:
            ...
            if created:
                self.stdout.write(f"  ✅ Created supplier: {supplier.code} - {supplier.name}")
            else:
                self.stdout.write(f"  ℹ️  Supplier already exists: {supplier.code} - {supplier.name}")
        ...
        for asset_data in assets_data:
            ...
            if existing:
                self.stdout.write(f"  ℹ️  Asset already exists: {existing.asset_code} - {existing.asset_name}")
                continue
            ...
            assets_created += 1
            self.stdout.write(f"  ✅ Created asset: {asset.asset_code} - {asset.asset_name}")

        self.stdout.write(f"\n📊 Summary:")
        self.stdout.write(f"  - Categories: {len(categories)}")
        self.stdout.write(f"  - Locations: {len(locations)}")
        self.stdout.write(f"  - Suppliers: {len(suppliers)}")
        self.stdout.write(f"  - New Assets Created: {assets_created}")

        # Verify final counts
        self.stdout.write(f"\n🔍 Current Database Counts:")
        self.stdout.write(f"  - AssetCategory: {AssetCategory.objects.filter(organization=org, is_deleted=False).count()}")
        self.stdout.write(f"  - Location: {Location.objects.filter(organization=org, is_deleted=False).count()}")
        self.stdout.write(f"  - Supplier: {Supplier.objects.filter(organization=org, is_deleted=False).count()}")
        self.stdout.write(f"  - Asset: {Asset.objects.filter(organization=org, is_deleted=False).count()}")
