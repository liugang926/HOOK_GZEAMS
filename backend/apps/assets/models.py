...
def _fallback_status_label(value):
    """Humanize status codes when dictionary metadata is unavailable."""
    if not value:
        return value
...
class Asset(BaseModel):
    ...
    source_receipt = models.ForeignKey(
        'lifecycle.AssetReceipt',
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name='generated_assets',
        help_text='Source asset receipt'
    )
    source_receipt_item = models.ForeignKey(
        'lifecycle.AssetReceiptItem',
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name='generated_assets',
        help_text='Source asset receipt item'
    )
    source_purchase_request = models.ForeignKey(
    ...
    def get_status_label(self, lang=None):
        ...
        try:
            ...
            label = DictionaryService.get_label(
                'ASSET_STATUS',
                self.asset_status,
                organization_id=self.organization_id,
                lang=lang or get_language()
            )
            return label if label != self.asset_status else _fallback_status_label(self.asset_status)
        except Exception:
            return _fallback_status_label(self.asset_status)
...
class AssetPickup(BaseModel):
    ...
    def get_status_label(self, lang=None):
        ...
        try:
            ...
            return label if label != self.status else _fallback_status_label(self.status)
        except Exception:
            return _fallback_status_label(self.status)
...
class ReturnItem(BaseModel):
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['organization', 'project_allocation']),
        ]
    ...
    project_allocation = models.ForeignKey(
...
class AssetLoan(BaseModel):
    ...
    def overdue_days(self):
        ...
        from django.utils import timezone
        if self.is_overdue():
            return (timezone.localdate() - self.expected_return_date).days
        return 0
