...
class AssetReturnServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.project = AssetProject.objects.create(
            organization=self.org,
            project_name='Asset Return Project',
    ...
    def test_create_return_links_project_allocation_to_return_item(self):
        """Test creating return order with linked project allocation context."""
        data = {
            'return_date': date.today(),
            'return_reason': 'Project completed',
            'return_location': self.location,
        ...
        line_item = return_order.items.get()
        self.assertEqual(line_item.project_allocation_id, self.project_allocation.id)

    def test_confirm_return_marks_linked_project_allocation_returned(self):
        """Test confirming return updates linked project allocation status."""
        return_order = AssetReturn.objects.create(
            organization=self.org,
            returner=self.user,
            return_date=date.today(),
        ...
        self.service.confirm_return(str(return_order.id), self.admin)

        self.project_allocation.refresh_from_db()
        self.project.refresh_from_db()
        self.assertEqual(self.project_allocation.return_status, 'returned')
        self.assertEqual(self.project_allocation.actual_return_date, date.today())
        self.assertIn('Returned via', self.project_allocation.notes)
