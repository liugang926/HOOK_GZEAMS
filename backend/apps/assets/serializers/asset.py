...
class AssetDetailSerializer(BaseModelWithAuditSerializer):
    ...
    # Display Fields
    asset_status_display = serializers.SerializerMethodField()
    source_receipt_no = serializers.CharField(source='source_receipt.receipt_no', read_only=True, allow_null=True)
    source_purchase_request_no = serializers.CharField(source='source_purchase_request.request_no', read_only=True, allow_null=True)
    source_receipt_item_sequence = serializers.IntegerField(source='source_receipt_item.sequence', read_only=True, allow_null=True)
    ...
    class Meta(BaseModelWithAuditSerializer.Meta):
        ...
        fields = BaseModelWithAuditSerializer.Meta.fields + [
            ...
            # Source Traceability
            'source_receipt', 'source_receipt_no',
            'source_receipt_item', 'source_receipt_item_sequence',
...
class AssetStatusSerializer(serializers.Serializer):
    ...
    def validate_new_status(self, value):
        ...
        from apps.system.services import DictionaryService
        if DictionaryService.validate_value('ASSET_STATUS', value):
            return value

        if value in self.LEGACY_STATUS_VALUES:
            return value
...
class AssetSerializer(BaseModelSerializer):
    ...
    source_receipt_no = serializers.CharField(
        source='source_receipt.receipt_no',
        read_only=True,
        allow_null=True
    )
    source_purchase_request_no = serializers.CharField(
        source='source_purchase_request.request_no',
        read_only=True,
        allow_null=True
    )
    tags = serializers.SerializerMethodField()
