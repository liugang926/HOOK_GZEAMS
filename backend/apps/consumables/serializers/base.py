...
class ConsumableStockAdjustmentSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if transaction_type == TransactionType.INVENTORY_ADD and quantity < 0:
            raise serializers.ValidationError({
                'quantity': 'Inventory addition requires a positive quantity.'
            })

        if transaction_type == TransactionType.INVENTORY_REDUCE:
            if quantity < 0:
                raise serializers.ValidationError({
                    'quantity': 'Inventory reduction expects a positive quantity.'
                })
