...
class ConsumablePurchase(BaseModel):
    ...
    def _generate_purchase_no(self):
        """Generate purchase order number using SequenceService with legacy fallback."""
        try:
            ...
            last_purchase = ConsumablePurchase.all_objects.filter(
                purchase_no__startswith=f"CP{prefix}"
            ).order_by('-purchase_no').first()
            if last_purchase:
                seq = int(last_purchase.purchase_no[-4:]) + 1
            else:
                seq = 1
            return f"CP{prefix}{seq:04d}"
...
class ConsumableIssue(BaseModel):
    ...
    def _generate_issue_no(self):
        """Generate issue order number using SequenceService with legacy fallback."""
        try:
            from apps.system.services import SequenceService
            return SequenceService.get_next_value(
                'CONSUMABLE_ISSUE_NO',
            ...
            prefix = timezone.now().strftime('%Y%m')
            last_issue = ConsumableIssue.all_objects.filter(
                issue_no__startswith=f"CI{prefix}"
            ).order_by('-issue_no').first()
            if last_issue:
                seq = int(last_issue.issue_no[-4:]) + 1
            else:
                seq = 1
            return f"CI{prefix}{seq:04d}"
