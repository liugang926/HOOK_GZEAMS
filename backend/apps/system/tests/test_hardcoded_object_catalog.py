...
@pytest.mark.django_db
def test_line_item_catalog_names_are_human_readable():
    hardcoded_objects = {
        obj["code"]: obj
    ...
    assert hardcoded_objects["PickupItem"]["name"] == "领用明细"
    assert hardcoded_objects["TransferItem"]["name"] == "调拨明细"
    assert hardcoded_objects["ReturnItem"]["name"] == "归还明细"
    assert hardcoded_objects["LoanItem"]["name"] == "借用明细"
