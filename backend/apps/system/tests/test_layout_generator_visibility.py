...
def _section_types(layout_config: dict) -> list[str]:
    return [
...
@pytest.mark.django_db
def test_layout_generator_injects_workflow_progress_for_workflow_root_objects():
    business_object = BusinessObject.objects.create(
        code='WF_DOC',
    ...
    assert _section_types(form_layout)[0] == 'workflow-progress'
    assert _section_types(detail_layout)[0] == 'workflow-progress'
    assert form_layout['sections'][0]['statusFieldCode'] == 'status'
    assert detail_layout['sections'][0]['translationKey'] == 'common.documentWorkbench.sections.workflowProgress'


@pytest.mark.django_db
def test_layout_generator_skips_workflow_progress_for_detail_objects():
    business_object = BusinessObject.objects.create(
        code='WF_DETAIL',
    ...
    assert 'workflow-progress' not in _section_types(form_layout)
