...
class RelationQueryService:
    ...
    def list_relations(self, parent_object_code: str, locale: Optional[str] = None) -> list[dict]:
        ...
        for row in rows:
            ...
            target_label_zh = (
                self._resolve_target_object_label(target_bo, TranslationService.DEFAULT_LANGUAGE)
                if target_bo else row.target_object_code
            )
            target_label_en = (
                self._resolve_target_object_label(target_bo, 'en-US')
                if target_bo else row.target_object_code
            )
            ...
            relation_name_zh = self._resolve_relation_label(row, TranslationService.DEFAULT_LANGUAGE, target_label_zh)
            relation_name_en = self._resolve_relation_label(row, 'en-US', target_label_en)
            ...
            relation_translation_key = self._resolve_relation_translation_key(row)
            ...
            payload.append(
                {
                    ...
                    'relation_name_i18n': self._build_locale_text_map(
                        zh_value=relation_name_zh,
                        en_value=relation_name_en,
                    ),
                    'relation_translation_key': relation_translation_key,
                    'target_object_code': row.target_object_code,
                    'target_object_name': target_label_zh,
                    'target_object_name_en': target_label_en,
                    'target_object_locale_names': self._build_locale_text_map(
                        zh_value=target_label_zh,
                        en_value=target_label_en,
                    ),
                    'target_object_role': getattr(target_bo, 'object_role', 'root') if target_bo else 'root',
                    'target_allow_standalone_query': bool(getattr(target_bo, 'allow_standalone_query', True)) if target_bo else True,
                    'target_allow_standalone_route': bool(getattr(target_bo, 'allow_standalone_route', True)) if target_bo else True,
                    'target_inherit_permissions': bool(getattr(target_bo, 'inherit_permissions', False)) if target_bo else False,
                    'target_inherit_workflow': bool(getattr(target_bo, 'inherit_workflow', False)) if target_bo else False,
                    'target_inherit_status': bool(getattr(target_bo, 'inherit_status', False)) if target_bo else False,
                    'target_inherit_lifecycle': bool(getattr(target_bo, 'inherit_lifecycle', False)) if target_bo else False,
                    'relation_kind': row.relation_kind,
                    'relation_type': row.relation_type,
                    'display_mode': row.display_mode,
                    'detail_edit_mode': row.detail_edit_mode,
                    ...
                    'cascade_soft_delete': bool(row.cascade_soft_delete),
                    'detail_toolbar_config': row.detail_toolbar_config or {},
                    'detail_summary_rules': row.detail_summary_rules or [],
                    'detail_validation_rules': row.detail_validation_rules or [],
    ...
    def _resolve_relation_translation_key(self, relation: ObjectRelationDefinition) -> str:
        extra = relation.extra_config or {}
        for key in ('relation_translation_key', 'detail_region_translation_key', 'translation_key'):
            value = extra.get(key)
    ...
    def _build_locale_text_map(self, *, zh_value: Optional[str], en_value: Optional[str]) -> dict:
        values = {}
        if isinstance(zh_value, str) and zh_value.strip():
            values['zh-CN'] = zh_value.strip()
        if isinstance(en_value, str) and en_value.strip():
            values['en-US'] = en_value.strip()
        return values
