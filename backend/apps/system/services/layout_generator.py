...
class LayoutGenerator:
    ...
    @classmethod
    def _supports_default_workflow_progress(cls, business_object: BusinessObject) -> bool:
        return bool(
    ...
    @classmethod
    def _build_workflow_progress_section(cls) -> Dict[str, Any]:
        return {
            'id': 'workflow_progress',
            'name': 'workflow_progress',
            'type': 'workflow-progress',
            'title': dict(cls.WORKFLOW_PROGRESS_TITLE),
            'translationKey': 'common.documentWorkbench.sections.workflowProgress',
            'columns': 1,
            'column': 1,
            'position': 'main',
    ...
    @classmethod
    def _inject_default_document_sections(
        cls,
        business_object: BusinessObject,
        sections: List[Dict[str, Any]],
    ) -> List[Dict[str, Any]]:
        if not cls._supports_default_workflow_progress(business_object):
            return sections

        if any(str(section.get('type') or '').strip() == 'workflow-progress' for section in sections):
            return sections

        return [cls._build_workflow_progress_section(), *sections]
    ...
    def _generate_form_from_model_fields(
        ...
        return {
            'sections': cls._inject_default_document_sections(
                business_object,
                cls._build_form_sections(
