...
from apps.system.viewsets.object_router_relation_actions import ObjectRouterRelationActionsMixin
from apps.system.viewsets.object_router_read_actions import ObjectRouterReadActionsMixin
from apps.system.viewsets.object_router_runtime_actions import ObjectRouterRuntimeActionsMixin
...
class ObjectRouterViewSet(
    ObjectRouterRelationActionsMixin,
    ObjectRouterRuntimeActionsMixin,
    ObjectRouterReadActionsMixin,
    viewsets.ViewSet,
):
    """
    ...
    def _resolve_relation_runtime_field_code(
        self,
        *,
        relation_code: str,
        editable_fields: List[dict],
        ...
        normalized_relation_code = str(relation_code or '').strip()
        if not normalized_relation_code:
            return ''

        for field in [*(editable_fields or []), *(reverse_relations or [])]:
            if not isinstance(field, dict):
                continue
            field_code = str(field.get('field_code') or field.get('fieldCode') or field.get('code') or '').strip()
            component_props = field.get('component_props') or field.get('componentProps') or {}
            relation_candidate = str(
                component_props.get('relation_code')
                or component_props.get('relationCode')
                or field.get('relation_code')
                or field.get('relationCode')
                or ''
            ).strip()
            if relation_candidate == normalized_relation_code and field_code:
                return field_code
            if field_code == normalized_relation_code:
                return field_code

        if normalized_relation_code.endswith('_items'):
            for field in editable_fields or []:
                field_code = str(field.get('field_code') or field.get('fieldCode') or field.get('code') or '').strip()
                if field_code == 'items':
                    return field_code

        return normalized_relation_code

    def _build_aggregate_runtime_payload(
        self,
        *,
        ...
        for relation in relations or []:
            relation_type = str(relation.get('relation_type') or relation.get('relationType') or 'lookup').strip()
            if relation_type != 'master_detail':
                continue

            relation_code = str(relation.get('relation_code') or relation.get('relationCode') or '').strip()
            embedded_field_code = self._resolve_relation_runtime_field_code(
                relation_code=relation_code,
                editable_fields=editable_fields,
                reverse_relations=[],
            )
            field_code = self._resolve_relation_runtime_field_code(
                relation_code=relation_code,
                editable_fields=editable_fields,
                reverse_relations=reverse_relations,
            )
            detail_regions.append(self._with_camel_case_keys({
                'relation_code': relation_code,
                'field_code': field_code,
                'title': str(relation.get('relation_name') or relation_code).strip(),
                'title_en': str(relation.get('relation_name_en') or relation.get('relationNameEn') or relation.get('relation_name') or relation_code).strip(),
                'title_i18n': relation.get('relation_name_i18n') or relation.get('relationNameI18n') or {},
                'translation_key': str(
                    relation.get('relation_translation_key')
                    or relation.get('relationTranslationKey')
                    or ''
                ).strip(),
                'target_object_code': str(relation.get('target_object_code') or relation.get('targetObjectCode') or '').strip(),
                'target_object_name': str(relation.get('target_object_name') or relation.get('targetObjectName') or '').strip(),
                'target_object_name_en': str(relation.get('target_object_name_en') or relation.get('targetObjectNameEn') or '').strip(),
                'target_object_locale_names': relation.get('target_object_locale_names') or relation.get('targetObjectLocaleNames') or {},
                'target_object_role': str(relation.get('target_object_role') or relation.get('targetObjectRole') or 'root').strip() or 'root',
                'relation_kind': str(relation.get('relation_kind') or relation.get('relationKind') or '').strip(),
                'relation_type': relation_type,
                'display_mode': str(relation.get('display_mode') or relation.get('displayMode') or '').strip(),
                'display_tier': str(relation.get('display_tier') or relation.get('displayTier') or '').strip() or 'L2',
                'target_fk_field': str(relation.get('target_fk_field') or relation.get('targetFkField') or '').strip(),
                'detail_edit_mode': str(relation.get('detail_edit_mode') or relation.get('detailEditMode') or 'readonly_table').strip(),
                'cascade_soft_delete': bool(relation.get('cascade_soft_delete') or relation.get('cascadeSoftDelete')),
                'toolbar_config': relation.get('detail_toolbar_config') or relation.get('detailToolbarConfig') or {},
                'summary_rules': relation.get('detail_summary_rules') or relation.get('detailSummaryRules') or [],
                'validation_rules': relation.get('detail_validation_rules') or relation.get('detailValidationRules') or [],
                'show_in_form': bool(embedded_field_code) and field_code == embedded_field_code,
                'show_in_detail': bool(field_code),
                'allow_standalone_query': bool(
                    relation.get('target_allow_standalone_query')
                    if relation.get('target_allow_standalone_query') is not None
                    else relation.get('targetAllowStandaloneQuery')
                ),
                'allow_standalone_route': bool(
                    relation.get('target_allow_standalone_route')
                    if relation.get('target_allow_standalone_route') is not None
                    else relation.get('targetAllowStandaloneRoute')
                ),
                'inherit_permissions': bool(
                    relation.get('target_inherit_permissions')
                    if relation.get('target_inherit_permissions') is not None
                    else relation.get('targetInheritPermissions')
                ),
                'inherit_workflow': bool(
                    relation.get('target_inherit_workflow')
                    if relation.get('target_inherit_workflow') is not None
                    else relation.get('targetInheritWorkflow')
                ),
                'inherit_status': bool(
                    relation.get('target_inherit_status')
                    if relation.get('target_inherit_status') is not None
                    else relation.get('targetInheritStatus')
                ),
                'inherit_lifecycle': bool(
                    relation.get('target_inherit_lifecycle')
                    if relation.get('target_inherit_lifecycle') is not None
                    else relation.get('targetInheritLifecycle')
                ),
            }))

        return self._with_camel_case_keys({
            'object_code': str(business_object.code or '').strip(),
            'object_role': str(getattr(business_object, 'object_role', 'root') or 'root').strip() or 'root',
            'is_aggregate_root': str(getattr(business_object, 'object_role', 'root') or 'root').strip() == 'root' and bool(detail_regions),
            'is_detail_object': str(getattr(business_object, 'object_role', 'root') or 'root').strip() == 'detail',
    ...
    def actions(self, request, *args, **kwargs):
        ...
        from apps.lifecycle.services import LifecycleActionService

        actions = LifecycleActionService().list_actions(
    ...
    def sla(self, request, *args, **kwargs):
        """
        ...
        payload = ObjectSLABindingService().get_object_sla_summary(
    ...
    def execute_action(self, request, *args, **kwargs):
        ...
        """
        record_id = kwargs.get('id')
        action_code = str(kwargs.get('action_code') or '').strip()
        if not record_id or not action_code:
            return BaseResponse.error(
                'VALIDATION_ERROR',
                'object id and action code are required',
                http_status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            result = LifecycleActionService().execute_action(
                object_code=self._object_meta.code,
            ...
            message = getattr(exc, 'messages', None) or ['Validation failed']
            return BaseResponse.error(
            ...
            return BaseResponse.permission_denied(str(exc) or 'Permission denied')
        except Exception as exc:
            logger.exception(
                "execute_action failed. code=%s id=%s action=%s error=%s",
                getattr(self._object_meta, 'code', None),
                record_id,
                action_code,
                exc,
            )
            return BaseResponse.server_error('Failed to execute action')

        return BaseResponse.success(data=result)

    @staticmethod
    def _normalize_validation_details(exc: Exception) -> dict:
        if hasattr(exc, 'message_dict') and getattr(exc, 'message_dict'):
            return getattr(exc, 'message_dict')
        ...
        if hasattr(exc, 'messages') and getattr(exc, 'messages'):
            return {'non_field_errors': getattr(exc, 'messages')}
        return {'non_field_errors': [str(exc)]}

    def _get_aggregate_document_service(self) -> AggregateDocumentService:
        return AggregateDocumentService()

    def document(self, request, *args, **kwargs):
        """
        Get unified aggregate document payload for supported master-detail roots.

        GET /api/system/objects/{code}/{id}/document/?mode={edit|readonly}
        """
        record_id = kwargs.get('id')
        if not record_id:
            return BaseResponse.error(
                'VALIDATION_ERROR',
                'common.messages.invalidRequest',
                details={'id': ['Record id is required']},
                http_status=status.HTTP_400_BAD_REQUEST,
            )

        service = self._get_aggregate_document_service()
        if not service.supports_object(getattr(self._object_meta, 'code', '')):
            return BaseResponse.error(
                code='NOT_FOUND',
        ...
        page_mode = str(request.query_params.get('mode') or 'readonly').strip() or 'readonly'
        organization_id = getattr(request, 'organization_id', None) or getattr(request.user, 'organization_id', None)

        try:
            payload = service.get_document(
                object_code=self._object_meta.code,
                record_id=str(record_id),
                user=request.user,
                organization_id=str(organization_id) if organization_id else None,
            ...
            return BaseResponse.validation_error(self._normalize_validation_details(exc))
    ...
    def document_create(self, request, *args, **kwargs):
        """
        Create a supported aggregate document through the unified master/detail contract.
        ...
        try:
            instance = service.create_document(
                object_code=self._object_meta.code,
                payload=payload,
                user=request.user,
                organization_id=str(organization_id) if organization_id else None,
            ...
            self._safe_log_create(request, instance)
            response_payload = service.build_document_response(
                object_code=self._object_meta.code,
                instance=instance,
                organization_id=str(organization_id) if organization_id else None,
                page_mode='edit',
        ...
        return BaseResponse.success(data=response_payload, http_status=status.HTTP_201_CREATED)

    def document_update(self, request, *args, **kwargs):
        """
        Full update through the unified aggregate document contract.

        PUT /api/system/objects/{code}/{id}/document/
        """
        return self._document_update(request, partial=False, *args, **kwargs)

    def document_partial_update(self, request, *args, **kwargs):
        """
        Partial update through the unified aggregate document contract.

        PATCH /api/system/objects/{code}/{id}/document/
        """
        return self._document_update(request, partial=True, *args, **kwargs)

    def _document_update(self, request, *args, partial: bool = False, **kwargs):
        record_id = kwargs.get('id')
        if not record_id:
            return BaseResponse.error(
                'VALIDATION_ERROR',
        ...
        organization_id = getattr(request, 'organization_id', None) or getattr(request.user, 'organization_id', None)
        normalized_org_id = str(organization_id) if organization_id else None
        payload = request.data if isinstance(request.data, dict) else {}

        before_instance = self._load_delegate_instance(record_id)
        before_snapshot = ActivityLogService.snapshot_instance(
            before_instance,
            fields=self._extract_changed_fields(request),
        ) if before_instance is not None else {}
        ...
        return BaseResponse.success(data=response_payload)
    ...
    def _legacy_relationship_graph(self, request, *args, **kwargs):
        """
    ...
    def _legacy_related(self, request, *args, **kwargs):
        """
    ...
    def _build_relation_runtime_fields(
        ...
        *,
        ...
        for item in relations:
            ...
            relation_name_en = str(item.get('relation_name_en') or item.get('relationNameEn') or relation_name).strip()
            ...
            relation_type = str(item.get('relation_type') or item.get('relationType') or 'lookup').strip() or 'lookup'
            ...
            if is_line_item_relation:
                ...
                component_props = {
                    ...
                    'detail_edit_mode': item.get('detail_edit_mode') or item.get('detailEditMode') or 'inline_table',
                    'detailEditMode': item.get('detail_edit_mode') or item.get('detailEditMode') or 'inline_table',
                    'title_i18n': item.get('relation_name_i18n') or item.get('relationNameI18n') or {},
                    'titleI18n': item.get('relation_name_i18n') or item.get('relationNameI18n') or {},
                    'translation_key': item.get('relation_translation_key') or item.get('relationTranslationKey') or '',
                    'translationKey': item.get('relation_translation_key') or item.get('relationTranslationKey') or '',
                    'toolbar_config': item.get('detail_toolbar_config') or item.get('detailToolbarConfig') or {},
                    'toolbarConfig': item.get('detail_toolbar_config') or item.get('detailToolbarConfig') or {},
                    'summary_rules': item.get('detail_summary_rules') or item.get('detailSummaryRules') or [],
                    'summaryRules': item.get('detail_summary_rules') or item.get('detailSummaryRules') or [],
                    'validation_rules': item.get('detail_validation_rules') or item.get('detailValidationRules') or [],
                    'validationRules': item.get('detail_validation_rules') or item.get('detailValidationRules') or [],
                ...
                payload.append(self._with_camel_case_keys({
                    **self._build_field_identifier_payload('items', force_strict=strict_identifier),
                    ...
                    'relation_type': relation_type,
                    'detail_edit_mode': item.get('detail_edit_mode') or item.get('detailEditMode') or 'inline_table',
                    'cascade_soft_delete': bool(item.get('cascade_soft_delete') or item.get('cascadeSoftDelete')),
                    'relation_name': relation_name,
                    'relation_name_en': relation_name_en,
                    'relation_code': relation_code,
                    'display_tier': display_tier,
                    'related_fields': related_fields,
                    'detail_toolbar_config': item.get('detail_toolbar_config') or item.get('detailToolbarConfig') or {},
                    'detail_summary_rules': item.get('detail_summary_rules') or item.get('detailSummaryRules') or [],
                    'detail_validation_rules': item.get('detail_validation_rules') or item.get('detailValidationRules') or [],
                    'component_props': component_props,
            ...
            payload.append(self._with_camel_case_keys({
                **self._build_field_identifier_payload(relation_code, force_strict=strict_identifier),
                ...
                'relation_kind': relation_kind,
                'relation_type': relation_type,
                'detail_edit_mode': item.get('detail_edit_mode') or item.get('detailEditMode') or 'readonly_table',
                'cascade_soft_delete': bool(item.get('cascade_soft_delete') or item.get('cascadeSoftDelete')),
                'relation_name': relation_name,
                'relation_name_en': relation_name_en,
                'relation_code': relation_code,
