"""
ViewSets for FieldPermission model.
"""
...
class FieldPermissionViewSet(BaseModelViewSetWithBatch):
    """
    ViewSet for FieldPermission management.

    Provides CRUD operations for field-level permissions.
    ...
    queryset = FieldPermission.all_objects.filter(is_deleted=False)
    ...
    def get_serializer_class(self):
        ...
        if self.action == 'create':
            ...
            return FieldPermissionUpdateSerializer
        elif self.action == 'retrieve':
            return FieldPermissionDetailSerializer
    ...
    def by_content_type(self, request):
        """
        Get field permissions for a specific content type.
        ...
        field_perms = engine.get_field_permissions(content_type)
    ...
    def grant(self, request):
        """
        Grant field permission to user or role.
        ...
        Request body:
        {
            ...
            "mask_rule": "phone" (optional, if permission_type is masked)
        }
        """
        ...
        return Response({
            'success': True,
            'message': 'Field permission granted',
            'data': FieldPermissionDetailSerializer(instance).data
        }, status=status.HTTP_201_CREATED)

    @action(detail=False, methods=['post'])
    def revoke(self, request):
        """
        Revoke field permission from user or role.
        ...
        for perm_id in ids:
            try:
                instance = FieldPermission.objects.get(id=perm_id, is_deleted=False)
        ...
        return Response({
            'success': True,
            'message': f'Field permissions revoked: {succeeded} succeeded, {failed} failed',
            'summary': {
                'total': len(ids),
                'succeeded': succeeded,
                'failed': failed
