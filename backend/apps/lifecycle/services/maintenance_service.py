...
class MaintenanceService(BaseCRUDService):
    ...
    def create(self, data: dict, user):
        ...
        if asset is not None:
            has_open_maintenance = Maintenance.objects.filter(
                asset=asset,
                is_deleted=False,
            ).exclude(
                status__in=[MaintenanceStatus.COMPLETED, MaintenanceStatus.CANCELLED]
            ).exists()
            if has_open_maintenance:
                raise ValidationError({
                    'asset': f'Asset {asset.asset_code} already has an open maintenance record'
                })
        ...
        self._enter_asset_maintenance(instance, user)
        return instance
    ...
    def complete_work(self, maintenance_id: str, completion_data: dict, user=None):
        """
        ...
        self._restore_asset_after_maintenance(
            ...
            reason=f'Maintenance {maintenance.maintenance_no} completed',
        )
    ...
    def cancel(self, maintenance_id: str, reason: str = None, user=None):
        """
    ...
    def _enter_asset_maintenance(self, maintenance: Maintenance, user):
        asset = maintenance.asset
        if asset is None or asset.asset_status == 'maintenance':
            return

        custom_fields = dict(maintenance.custom_fields or {})
        custom_fields['asset_previous_status'] = asset.asset_status
        maintenance.custom_fields = custom_fields
        maintenance.save(update_fields=['custom_fields', 'updated_at'])

        self.asset_service.change_status(
            str(asset.id),
            'maintenance',
            user,
            reason=f'Maintenance {maintenance.maintenance_no} created',
        )

    def _restore_asset_after_maintenance(self, maintenance: Maintenance, user, reason: str):
        asset = maintenance.asset
        if asset is None or asset.asset_status != 'maintenance':
            return

        has_other_open_maintenance = Maintenance.objects.filter(
            asset=asset,
            is_deleted=False,
        ).exclude(
            id=maintenance.id
        ...
        if has_other_open_maintenance:
            return

        previous_status = str((maintenance.custom_fields or {}).get('asset_previous_status') or '').strip()
