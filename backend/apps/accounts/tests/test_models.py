...
class TestUser:
    ...
    def test_get_primary_organization_prefers_current_membership_over_stale_primary(
        self,
        db,
        organization,
        second_organization,
    ):
        """Current active organization should override a stale primary membership."""
        user = User.objects.create_user(
            username='currentwins',
            email='currentwins@example.com',
            password='testpass123'
        )
        UserOrganization.objects.create(
            user=user,
        ...
        user.current_organization = second_organization
        user.save(update_fields=['current_organization'])

        primary = user.get_primary_organization()

        user.refresh_from_db()
        first_membership = UserOrganization.objects.get(user=user, organization=organization)
        second_membership = UserOrganization.objects.get(
            user=user,
            organization=second_organization,
        )
        assert primary == second_organization
        assert first_membership.is_primary is False
        assert second_membership.is_primary is True
    ...
    def test_get_primary_organization_auto_assigns_superuser(self, db, organization):
        ...
        user.refresh_from_db()
        membership = UserOrganization.objects.get(user=user, organization=primary)
        assert primary is not None
        assert membership.role == 'admin'
        assert membership.is_active is True
        assert membership.is_primary is True
        assert user.current_organization == primary

    def test_get_primary_organization_materializes_legacy_organization_membership(
        self,
        db,
        organization,
    ):
        """Legacy users with only BaseModel.organization should receive a default membership."""
        user = User.objects.create_user(
            username='legacyorguser',
            email='legacyorguser@example.com',
            password='testpass123',
            organization=organization,
        )
