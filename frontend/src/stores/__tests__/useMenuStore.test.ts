...
describe('normalizeBusinessObjects', () => {
    ...
    it('normalizes camelCase / snake_case field variants', () => {
        ...
        expect(result[0].objectRole).toBe('detail')
        expect(result[0].isTopLevelNavigable).toBe(false)
        expect(result[0].allowStandaloneQuery).toBe(true)
        expect(result[0].allowStandaloneRoute).toBe(false)
        expect(result[0].inheritPermissions).toBe(true)
        expect(result[0].inheritWorkflow).toBe(true)
        expect(result[0].inheritStatus).toBe(true)
        expect(result[0].inheritLifecycle).toBe(true)
    })
...
describe('normalizeMenuGroups', () => {
    it('normalizes groups with items', () => {
                    ...
                    items: [
                        {
                            ...
                            icon: 'Document',
                            order: 1,
                            localeNames: { 'zh-CN': '资产', 'en-US': 'Asset' },
                        },
                    ],
                },
            ],
        ...
        expect(result[0].items[0].localeNames).toEqual({
            'zh-CN': '资产',
            'en-US': 'Asset',
        })
    })
