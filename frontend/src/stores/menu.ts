...
export function normalizeBusinessObjects(payload: AnyRecord): BusinessObject[] {
    ...
    for (const item of source) {
        ...
        normalized.push({
            ...
            objectRole: String(item?.objectRole || item?.object_role || 'root') as BusinessObject['objectRole'],
            isTopLevelNavigable:
                item?.isTopLevelNavigable === false || item?.is_top_level_navigable === false
                    ? false
                    : true,
            allowStandaloneQuery:
                item?.allowStandaloneQuery === false || item?.allow_standalone_query === false
                    ? false
                    : true,
            allowStandaloneRoute:
                item?.allowStandaloneRoute === false || item?.allow_standalone_route === false
                    ? false
                    : true,
            inheritPermissions:
                item?.inheritPermissions === true ||
                item?.inherit_permissions === true,
            inheritWorkflow:
                item?.inheritWorkflow === true ||
                item?.inherit_workflow === true,
            inheritStatus:
                item?.inheritStatus === true ||
                item?.inherit_status === true,
            inheritLifecycle:
                item?.inheritLifecycle === true ||
                item?.inherit_lifecycle === true,
        })
    }
    return normalized
}
    ...
    return source
        .map((group, groupIndex) => {
            ...
            const items: LocalMenuItem[] = itemsSource.map((item, itemIndex) => ({
                ...
                localeNames:
                    item?.localeNames && typeof item.localeNames === 'object'
                        ? Object.fromEntries(
                              Object.entries(item.localeNames as Record<string, unknown>)
...
export const useMenuStore = defineStore('menu', () => {
            ...
            .map((group) => {
                ...
                const filteredItems = group.items.filter(
                    (item) =>
                        ...
                        Object.values(item.localeNames || {}).some((text) =>
    ...
    function getItemLabel(item: LocalMenuItem): string {
        ...
        const localeNames = item.localeNames || {}
        const currentLocale = String(locale.value || 'zh-CN')
