...
describe('DynamicFormPage', () => {
    ...
    return mount(DynamicFormPage, {
      global: createDynamicFormGlobalOptions(
        defineComponent({
          ...
          setup(_props: DynamicFormStubProps, { expose }) {
            expose({
              validate: vi.fn().mockResolvedValue(true),
              getSubmitData: vi.fn().mockImplementation(() => {
                const nextPayload = { ...(_props.modelValue || {}) }
            ...
            return () => h(
              ...
              JSON.stringify(_props.modelValue || {})
            )
          },
        })
      ),
  ...
  it('applies route prefill and returns to the requested path after create', async () => {
    routeState.params = { code: 'ProjectAsset' }
    routeState.path = '/objects/ProjectAsset/create'
    routeState.query = {
      prefill_project: 'project-1',
      returnTo: '/objects/AssetProject/project-1',
    }
    getMetadataMock.mockResolvedValueOnce({
      code: 'ProjectAsset',
      name: 'Project Asset',
      ...
      module: 'Projects',
      fields: [
        { code: 'project', required: true },
        { code: 'asset', required: true },
      ],
      layouts: {},
      permissions: { view: true, add: true, change: true, delete: true },
      isHardcoded: true,
    ...
    expect(pushMock).toHaveBeenCalledWith('/objects/AssetProject/project-1')
  })
})
