 ...
 *   zh-CN  → label_zh → label_zh_cn → label → name
 *   en-US  → label_en → label_en_us → label → name
 *   other  → label → name
 *
 * Usage (pure, no store dependency — works in any context):
 *   getLocalizedLabel(field, 'zh-CN')  // returns '资产名称'
...
/** Map of locale code → ordered list of field keys to try */
...
export function getLocalizedLabel(field: Record<string, any>, locale?: LocaleCode): string {
    const resolvedLocale = locale || localStorage.getItem('locale') || 'zh-CN'
    const keys =
