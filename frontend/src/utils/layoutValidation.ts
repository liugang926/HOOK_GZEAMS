...
const VALID_SECTION_TYPES = ['section', 'tab', 'divider', 'collapse', 'column', 'detail-region'] as const
const VALID_SPANS = [1, 2, 3, 4, 6, 8, 12, 24] as const
const VALID_DETAIL_EDIT_MODES = ['inline_table', 'nested_form', 'readonly_table'] as const
...
function validateSection(section: any, path: string, errors: LayoutValidationError[]): void {
  ...
  switch (section.type) {
    ...
    case 'column':
      ...
      break
    case 'detail-region':
      validateDetailRegionSection(section, path, errors)
      break
  }
}
...
function validateDetailRegionSection(section: any, path: string, errors: LayoutValidationError[]): void {
  ...
  if (!relationCode) {
    errors.push({ path, message: missingRequiredFieldMessage('relationCode') })
  }
  ...
  if (detailEditMode && !VALID_DETAIL_EDIT_MODES.includes(detailEditMode as typeof VALID_DETAIL_EDIT_MODES[number])) {
    errors.push({
      path: `${path}.detailEditMode`,
      message: lt('common.layoutValidation.invalidType', {
        type: detailEditMode,
        types: VALID_DETAIL_EDIT_MODES.join(', ')
      })
    })
  }

  const toolbarConfig = section.toolbarConfig ?? section.toolbar_config
  if (toolbarConfig !== undefined && (toolbarConfig === null || typeof toolbarConfig !== 'object' || Array.isArray(toolbarConfig))) {
    errors.push({ path: `${path}.toolbarConfig`, message: lt('common.layoutValidation.mustBeObject', { field: 'toolbarConfig' }) })
  }

  const summaryRules = section.summaryRules ?? section.summary_rules
  if (summaryRules !== undefined && !Array.isArray(summaryRules)) {
    errors.push({ path: `${path}.summaryRules`, message: lt('common.layoutValidation.mustBeArray', { field: 'summaryRules' }) })
  }

  const validationRules = section.validationRules ?? section.validation_rules
  if (validationRules !== undefined && !Array.isArray(validationRules)) {
    errors.push({ path: `${path}.validationRules`, message: lt('common.layoutValidation.mustBeArray', { field: 'validationRules' }) })
  }
}
  ...
  if (layoutType === 'form' || layoutType === 'detail') {
    ...
    for (const section of sections) {
      if (section.type === 'tab') {
        ...
        const detailFieldCode = String(
          section.fieldCode || section.field_code || section.relationCode || section.relation_code || ''
        ).trim()
        if (detailFieldCode) return false
