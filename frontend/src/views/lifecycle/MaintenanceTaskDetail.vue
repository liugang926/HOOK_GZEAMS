    ...
    <div v-else-if="detail" class="detail-wrapper">
      ...
      <div class="page-header">
        <div class="header-left">
          ...
          <h2 class="page-title">{{ detail.taskNo || $t('assets.lifecycle.maintenanceTask.detailTitle') }}</h2>
      ...
      <el-card class="info-card">
        <el-descriptions :column="3" border>
          <el-descriptions-item :label="$t('assets.lifecycle.maintenanceTask.columns.taskNo')">{{ detail.taskNo }}</el-descriptions-item>
          <el-descriptions-item :label="$t('assets.lifecycle.maintenanceTask.columns.asset')">{{ detail.assetDisplay || '—' }}</el-descriptions-item>
          <el-descriptions-item :label="$t('assets.lifecycle.maintenanceTask.columns.scheduledDate')">{{ detail.scheduledDate }}</el-descriptions-item>
          <el-descriptions-item :label="$t('assets.lifecycle.maintenanceTask.columns.maintenanceContent')" :span="3">{{ detail.maintenanceContent }}</el-descriptions-item>
          <el-descriptions-item v-if="detail.executionResult" :label="$t('assets.lifecycle.maintenanceTask.form.executionResult')" :span="3">{{ detail.executionResult }}</el-descriptions-item>
          <el-descriptions-item v-if="detail.verifyResult" :label="$t('assets.lifecycle.maintenanceTask.form.verifyResult')" :span="3">{{ detail.verifyResult }}</el-descriptions-item>
        </el-descriptions>
      </el-card>
    </div>

    <!-- Execute Dialog -->
    <el-dialog v-model="executeDialog" :title="$t('assets.lifecycle.maintenanceTask.dialog.executeTitle')" width="480px" destroy-on-close>
      <el-form :model="executeForm" label-width="120px">
        <el-form-item :label="$t('assets.lifecycle.maintenanceTask.form.executionResult')">
          <el-input v-model="executeForm.result" type="textarea" :rows="4" :placeholder="$t('assets.lifecycle.maintenanceTask.dialog.executionResultPlaceholder')" />
        </el-form-item>
        <el-form-item :label="$t('assets.lifecycle.maintenanceTask.form.actualHours')">
          <el-input-number v-model="executeForm.actualHours" :min="0" :precision="1" style="width:100%" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="executeDialog = false">{{ $t('common.actions.cancel') }}</el-button>
        <el-button type="primary" @click="handleExecute">{{ $t('common.actions.confirm') }}</el-button>
      </template>
    </el-dialog>

    <!-- Verify Dialog -->
    <el-dialog v-model="verifyDialog" :title="$t('assets.lifecycle.maintenanceTask.dialog.verifyTitle')" width="480px" destroy-on-close>
      <el-form :model="verifyForm" label-width="120px">
        <el-form-item :label="$t('assets.lifecycle.maintenanceTask.form.verifyResult')">
          <el-input v-model="verifyForm.result" type="textarea" :rows="4" :placeholder="$t('assets.lifecycle.maintenanceTask.dialog.verifyResultPlaceholder')" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="verifyDialog = false">{{ $t('common.actions.cancel') }}</el-button>
        <el-button type="primary" @click="handleVerify">{{ $t('common.actions.confirm') }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
