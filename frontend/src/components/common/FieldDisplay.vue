<template>
    ...
    >
      ...
      >
        <el-table-column
          ...
          :align="column.align"
          :fixed="column.fixed"
          :show-overflow-tooltip="column.tooltipTemplate ? false : column.showOverflowTooltip"
        >
          <template #default="{ row }">
            <el-tooltip
              v-if="resolveSubTableTooltip(row, column)"
              :content="resolveSubTableTooltip(row, column)"
              placement="top"
            >
              <span>{{ formatSubTableCell(row, column) }}</span>
            </el-tooltip>
            <span v-else>{{ formatSubTableCell(row, column) }}</span>
          </template>
        </el-table-column>
      </el-table>
  ...
  <template v-else-if="displayType === 'workflow_progress'">
...
const displayType = computed(() =>
  normalizeFieldType(props.field?.type || props.field?.editorType || props.field?.fieldType || 'text')
)
...
const subTableColumns = computed<SubTableColumn[]>(() => {
  ...
  const rawColumns = Array.isArray(fieldProps.columns)
    ? fieldProps.columns
    : (Array.isArray(fieldProps.relatedFields)
      ? fieldProps.relatedFields
      : (Array.isArray(fieldProps.related_fields) ? fieldProps.related_fields : []))
  const fromConfig: SubTableColumn[] = rawColumns
    .map((column: any): SubTableColumn | null => {
      ...
      const minWidth = Number(column.minWidth ?? column.min_width)
      return {
        key,
        label,
        width: Number.isFinite(width) && width > 0 ? Math.round(width) : undefined,
        minWidth: Number.isFinite(minWidth) && minWidth > 0 ? Math.round(minWidth) : undefined,
        align: ['left', 'center', 'right'].includes(String(column.align || '').trim().toLowerCase())
          ? String(column.align).trim().toLowerCase() as 'left' | 'center' | 'right'
          : undefined,
        fixed: ['left', 'right'].includes(String(column.fixed || '').trim().toLowerCase())
          ? String(column.fixed).trim().toLowerCase() as 'left' | 'right'
          : undefined,
        formatter: String(column.formatter || column.displayFormatter || '').trim() || undefined,
        emptyText: String(column.emptyText ?? column.empty_text ?? '').trim() || undefined,
        tooltipTemplate: String(column.tooltipTemplate ?? column.tooltip_template ?? '').trim() || undefined,
        showOverflowTooltip:
          column.ellipsis === true ||
          column.showOverflowTooltip === true ||
          column.show_overflow_tooltip === true
      }
    })
    .filter((column): column is SubTableColumn => column !== null)
...
const formatSubTableBoolean = (value: any): string => {
  if (value === true || value === 1) return t('common.yes')
  if (value === false || value === 0) return t('common.no')
  if (typeof value === 'string') {
    const normalized = value.trim().toLowerCase()
    if (['true', '1', 'yes', 'y'].includes(normalized)) return t('common.yes')
    if (['false', '0', 'no', 'n'].includes(normalized)) return t('common.no')
  }
  return safeText(value)
}

const formatSubTableValueWithFormatter = (value: any, column: SubTableColumn): string => {
  const formatter = String(column.formatter || '').trim().toLowerCase()
  if (!formatter) return safeText(value)
  ...
  switch (formatter) {
    ...
    case 'number': {
      ...
      return Number.isFinite(numeric) ? new Intl.NumberFormat().format(numeric) : safeText(value)
    }
    case 'date': {
      const formatted = formatDate(value, 'YYYY-MM-DD')
      return formatted && formatted !== 'Invalid Date' ? formatted : safeText(value)
    }
    case 'datetime': {
    ...
    case 'boolean':
      return formatSubTableBoolean(value)
    default:
      return safeText(value)
  }
}

const formatSubTableCell = (row: Record<string, any>, column: SubTableColumn): string => {
  const rawValue = row?.[column.key]
  if (rawValue === undefined || rawValue === null || rawValue === '') {
    return column.emptyText || '-'
  }

  const formatted = formatSubTableValueWithFormatter(rawValue, column)
...
const resolveSubTableTooltip = (row: Record<string, any>, column: SubTableColumn): string => {
  const template = String(column.tooltipTemplate || '').trim()
  if (!template) return ''

  const rawValue = row?.[column.key]
  const formattedValue = formatSubTableCell(row, column)
  const rendered = template.replace(/\{([^}]+)\}/g, (_, token: string) => {
    const normalized = String(token || '').trim()
    if (!normalized) return ''
    if (normalized === 'value' || normalized === 'formatted') {
      return formattedValue === '-' ? '' : formattedValue
    }
    if (normalized === 'rawValue' || normalized === 'raw') {
      return rawValue === undefined || rawValue === null || rawValue === '' ? '' : safeText(rawValue)
    }
    const candidate = row?.[normalized]
    return candidate === undefined || candidate === null || candidate === '' ? '' : safeText(candidate)
  }).trim()
...
const workflowStatusFieldCode = computed(() => {
  const componentProps = props.field.componentProps || {}
  const candidate = String(
    componentProps.statusFieldCode ||
    componentProps.status_field_code ||
    'status'
  ).trim()
...
const workflowSteps = computed<DocumentWorkflowProgressStepDefinition[] | null>(() => {
  const componentProps = props.field.componentProps || {}
  return Array.isArray(componentProps.steps)
    ? componentProps.steps as DocumentWorkflowProgressStepDefinition[]
    : null
})

const workflowCurrentStatus = computed(() => {
  const record = props.record || {}
  const fromRecord = record?.[workflowStatusFieldCode.value]
  if (fromRecord !== undefined && fromRecord !== null && String(fromRecord).trim()) {
    return String(fromRecord).trim()
  }
  if (props.value !== undefined && props.value !== null && String(props.value).trim()) {
...
const workflowProgress = computed(() => {
  return resolveDocumentWorkflowProgress({
    objectCode: String(props.field?.objectCode || props.field?.componentProps?.objectCode || '').trim(),
    currentStatus: workflowCurrentStatus.value,
