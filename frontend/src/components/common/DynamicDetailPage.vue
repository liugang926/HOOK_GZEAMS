...
import type { RuntimeAggregate } from '@/types/runtime'
...
function collectAggregateDetailFieldCodes(
  ...
  const regionCodes = detailRegions
    .map((region) => String(region?.fieldCode || '').trim())
    .filter(Boolean)
  if (regionCodes.length > 0) return [...new Set(regionCodes)]

  return fields
    .map((field) => String(field?.code || field?.fieldCode || field?.field_code || '').trim())
    .filter((code) => code === 'items')
}

function runtimeLayoutContainsFieldCode(layoutSections: any[], fieldCode: string): boolean {
  const normalizedFieldCode = String(fieldCode || '').trim()
  if (!normalizedFieldCode) return false

  for (const section of Array.isArray(layoutSections) ? layoutSections : []) {
    ...
    if (sectionType === 'detail-region') {
      ...
      if (detailFieldCode === normalizedFieldCode) return true
      continue
    }
    if (sectionType === 'tab') {
      for (const tab of Array.isArray(section?.tabs) ? section.tabs : []) {
        const fields = Array.isArray(tab?.fields) ? tab.fields : []
        if (fields.some((field: any) => String(field?.fieldCode || field?.field_code || field?.code || '').trim() === normalizedFieldCode)) {
          return true
        }
      }
      continue
...
function injectAggregateDetailFieldsIntoRuntimeSections(
  layoutSections: any[],
  fields: Array<Record<string, any>>,
  aggregate: RuntimeAggregate | null | undefined
): any[] {
  const detailFieldCodes = new Set(collectAggregateDetailFieldCodes(aggregate, fields))
  if (detailFieldCodes.size === 0) return layoutSections

  const missingFields = fields
    .filter((field) => detailFieldCodes.has(String(field?.code || '').trim()))
    .filter((field) => !runtimeLayoutContainsFieldCode(layoutSections, String(field?.code || '').trim()))
    .map((field) => ({
      fieldCode: String(field?.code || '').trim(),
      code: String(field?.code || '').trim(),
      label: field?.label || field?.name || String(field?.code || '').trim(),
      span: 24,
      componentProps: {
  ...
  if (missingFields.length === 0) return layoutSections

  const nextSections = JSON.parse(JSON.stringify(layoutSections || [])) as any[]
  let inserted = false
  for (const section of nextSections) {
    ...
    if (sectionType === 'tab') {
      ...
      const currentFields = Array.isArray(targetTab?.fields) ? [...targetTab.fields] : []
      targetTab.fields = [...currentFields, ...missingFields]
      inserted = true
      break
    }
    ...
    if (sectionType === 'collapse') {
      ...
      const currentFields = Array.isArray(targetItem?.fields) ? [...targetItem.fields] : []
      targetItem.fields = [...currentFields, ...missingFields]
      inserted = true
      break
    }

    const currentFields = Array.isArray(section?.fields) ? [...section.fields] : []
    section.fields = [...currentFields, ...missingFields]
    inserted = true
    break
  }
...
async function loadRuntimeLayout(): Promise<void> {
  ...
  try {
    ...
    if (editable.length > 0) {
      ...
      const sections = injectAggregateDetailFieldsIntoRuntimeSections(
        compiled.layoutConfig?.sections || [],
        normalizedEditable as Array<Record<string, any>>,
        runtime.aggregate
      )
