...
import type { PageLayoutMutationPayload } from '@/api/system/pageLayout'
...
function toLayoutConfigRecord(value: unknown): Record<string, unknown> | null {
  return value && typeof value === 'object' ? value as Record<string, unknown> : null
}

export function useDesignerPersistenceActions(options: UseDesignerPersistenceActionsOptions) {
  function resolvePersistMode(): LayoutMode {
    return options.props.mode === 'readonly' || options.props.mode === 'search'
      ? options.props.mode
      : 'edit'
  }
  ...
  async function resolveSharedEditLayout(readonlySeed?: LayoutConfig): Promise<AnyRecord> {
    ...
    const createPayload: PageLayoutMutationPayload = {
      layoutCode: `${options.props.objectCode}_edit_shared_${Date.now()}`,
      layoutName: `${options.props.objectCode} Edit Shared`,
      mode: 'edit',
      business_object: sourceBusinessObject,
      status: 'draft',
      isDefault: false,
      layoutConfig: baseLayoutConfig || fallbackSeed
    }

    const created = options.unwrapData(await pageLayoutApi.create(createPayload) as ApiDataEnvelope<AnyRecord> | AnyRecord)
    if (!created?.id) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (options.isReadonlyMode.value && options.props.objectCode) {
        ...
        await pageLayoutApi.partialUpdate(options.props.layoutId, data)
      } else {
    ...
    try {
      ...
      if (options.isReadonlyMode.value && options.props.objectCode) {
        ...
        const createResult = options.unwrapData(await pageLayoutApi.create({
          ...
          business_object: options.props.businessObjectId,
          status: 'draft'
        }) as ApiDataEnvelope<{ id: string }> | { id: string })
  ...
  async function handleReset() {
    ...
    try {
      ...
      const backendConfig = toLayoutConfigRecord(payload?.layoutConfig || payload?.layout_config || payload?.layout || null)
      const backendSections = Array.isArray(backendConfig?.sections) ? backendConfig.sections : []
      const backendColumns = Array.isArray(backendConfig?.columns) ? backendConfig.columns : []
