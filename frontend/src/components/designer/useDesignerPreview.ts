...
interface UseDesignerPreviewOptions {
  ...
  resolveLayoutFieldMinHeight: (field: Partial<LayoutField> | null | undefined) => number
...
export function useDesignerPreview(options: UseDesignerPreviewOptions) {
  ...
  function fieldToDesignDisplayField(field: LayoutField): DetailField {
    const runtimeField = toRuntimeFieldFromLayout(
      ...
      options.availableFields.value as Array<Record<string, unknown>>
    )
  ...
  const designerRenderSections = computed<DesignerRenderSection[]>(() => {
    return (options.layoutConfig.value.sections || []).map((section: LayoutSection) => {
      ...
      const sectionTitle =
        resolveTranslatableText(section.title, locale.value as 'zh-CN' | 'en-US') ||
  ...
  const previewAuditInfo = computed(() => ({
    createdBy: options.sampleData.value.createdBy || options.sampleData.value.created_by || 'System',
    createdAt: options.sampleData.value.createdAt || options.sampleData.value.created_at || '2026-03-01 10:00:00',
    updatedBy: options.sampleData.value.updatedBy || options.sampleData.value.updated_by || 'System',
    updatedAt: options.sampleData.value.updatedAt || options.sampleData.value.updated_at || '2026-03-01 12:30:00'
  }))
