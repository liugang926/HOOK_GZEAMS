...
import type { RuntimeAggregate, RuntimeLayoutConfig } from '@/types/runtime'
...
export function useDynamicForm(
  ...
  const isAggregateDetailField = (
    field: Record<string, unknown> | null | undefined,
    detailFieldCodes: Set<string>
  ): boolean => {
    ...
    return detailFieldCodes.has(code) && (fieldType === 'sub_table' || fieldType === 'subtable')
  }

  const collectAggregateDetailFieldCodes = (
    aggregate: RuntimeAggregate | null | undefined,
    fields: FieldDefinition[]
  ): string[] => {
    const regions = Array.isArray(aggregate?.detailRegions) ? aggregate?.detailRegions || [] : []
  ...
  const injectAggregateDetailFieldsIntoLayout = (
    rawLayout: Record<string, unknown> | null,
    fields: FieldDefinition[],
    aggregate: RuntimeAggregate | null = null
  ): Record<string, unknown> | null => {
    if (layoutCode !== 'form') return rawLayout
    const detailFieldCodes = new Set(collectAggregateDetailFieldCodes(aggregate, fields))
    if (detailFieldCodes.size === 0) return rawLayout
    if (!rawLayout || typeof rawLayout !== 'object') return rawLayout
    const detailFields = fields.filter((field) => isAggregateDetailField(field as unknown as Record<string, unknown>, detailFieldCodes))
    const missingFields = detailFields.filter((field) => !layoutContainsFieldCode(rawLayout, String(field.code || '').trim()))
    if (missingFields.length === 0) return rawLayout
    ...
    const injectedFields = missingFields.map((field) => ({
    ...
    for (const section of sections) {
      ...
      const fields = Array.isArray(section?.fields) ? [...(section.fields as Array<Record<string, unknown>>)] : []
      section.fields = [...fields, ...injectedFields]
      inserted = true
      break
    }
  ...
  async function loadMetadata(): Promise<void> {
    ...
    try {
      ...
      if (layoutConfig && availableFields) {
        ...
        const normalizedPreviewLayout = injectAggregateDetailFieldsIntoLayout(
          layoutConfig,
      ...
      if (fieldsData && Array.isArray(fieldsData)) {
        ...
        const normalizedLayoutData = injectAggregateDetailFieldsIntoLayout(
          layoutData,
          transformedFields,
          resolved.aggregate
        )
