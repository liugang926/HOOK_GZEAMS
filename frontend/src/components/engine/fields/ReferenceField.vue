...
type ReferenceOption = AnyRecord & { id: string }
type ReferenceFieldValue = string | number | AnyRecord | unknown[] | null | undefined
type SelectRefLike = {
  blur?: () => void
}
...
const searchOptions = ref<ReferenceOption[]>([])
const recentOptions = ref<ReferenceOption[]>([])
const currentValueObjects = ref<ReferenceOption[]>([])
const searchKeyword = ref('')
const lookupDialogVisible = ref(false)

const normalizeReferenceOption = (input: unknown): ReferenceOption | null => {
  const normalized = normalizeOption(input)
  return normalized ? normalized as ReferenceOption : null
}

const normalizeReferenceOptions = (items: unknown[]): ReferenceOption[] => {
  return items
    .map((item) => normalizeReferenceOption(item))
    .filter((item): item is ReferenceOption => !!item)
}

const toReferenceFieldValue = (value: unknown): ReferenceFieldValue => {
...
const allOptions = computed(() => {
  ...
  for (const item of merged) {
    const normalized = normalizeReferenceOption(item)
    if (!normalized) continue
    const existing = map.get(normalized.id)
    map.set(normalized.id, existing ? { ...normalized, ...existing, id: normalized.id } : normalized)
  }
...
const selectedSingleOption = computed<ReferenceOption | null>(() => {
...
const buildRecordHref = (item: ReferenceOption): string => {
...
const getItemLabel = (item: ReferenceOption) => {
  return resolveReferenceLabel(item, displayField.value) || String(item?.id || '')
}

const getItemSecondary = (item: ReferenceOption) => {
...
const loadRecentReferences = async () => {
  ...
  try {
    ...
    recentOptions.value = recentIds
      .map((id) => normalizeReferenceOption(resolved[id]))
      .filter((item): item is ReferenceOption => !!item)
  } catch (error) {
...
const searchReference = async (query = '') => {
  ...
  try {
    ...
    searchOptions.value = (res.results || res.items || [])
      .map((item: unknown) => normalizeReferenceOption(item))
      .filter((item: ReferenceOption | null): item is ReferenceOption => !!item)
  } finally {
...
const handleUpdate = (val: unknown) => {
  emit('update:modelValue', toReferenceFieldValue(val))
  ...
  const selectedObj = isMultiple.value
    ? allOptions.value.filter((item) => selectedIds.includes(item.id))
    : (allOptions.value.find((item) => item.id === selectedIds[0]) || null)
...
const handleLookupConfirm = (rows: AnyRecord[]) => {
  const normalizedRows = normalizeReferenceOptions(rows || [])
  ...
  currentValueObjects.value = Array.from(
    new Map<string, ReferenceOption>([...currentValueObjects.value, ...normalizedRows].map((item) => [item.id, item])).values()
  )
...
const fetchCurrentValue = async () => {
  const ids = extractReferenceIds(props.modelValue)
  const embeddedOptions = normalizeReferenceOptions(extractEmbeddedReferenceOptions(props.modelValue))
  ...
  try {
    ...
    currentValueObjects.value = normalizeReferenceOptions(
      mergeReferenceOptionsByIds(
        ids,
