...
const INVENTORY_TASK_ROUTE = '/objects/InventoryTask'
...
test.describe('Inventory Workflow', () => {
  ...
  test('should display inventory task list', async ({ page }) => {
    await page.goto(INVENTORY_TASK_ROUTE)
  ...
  test('should create new inventory task', async ({ page }) => {
    ...
    // Wait for form
    const createSurface = page.locator('.el-dialog, .el-drawer, form:not(.search-form)').first()
    const hasForm = await createSurface.isVisible({ timeout: 5000 }).catch(() => false)
    if (!hasForm) {
      return
    }
  ...
  test('should execute inventory task', async ({ page }) => {
    ...
    // Wait for table
    const tableContainer = page.locator('.el-table, .task-list, table').first()
