...
test.describe('Finance Voucher Detail Regression', () => {
  test('voucher list detail action should navigate and render business fields', async ({ page }) => {
    ...
    await expect(page.locator('.voucher-list-page')).toBeVisible({ timeout: 15000 })
    await expect(page.locator('.voucher-list-page')).toContainText(VOUCHER_NO, { timeout: 15000 })

    const voucherRow = page.locator('.el-table__row', { hasText: VOUCHER_NO }).first()
    await expect(voucherRow).toBeVisible({ timeout: 15000 })
    await voucherRow.locator('button').first().click()

    await expect(page).toHaveURL(new RegExp(`/finance/vouchers/${VOUCHER_ID}$`), { timeout: 15000 })
    ...
    await expect(page.locator('.voucher-detail-page')).toContainText('Regression voucher for detail rendering', { timeout: 15000 })
    await expect(page.locator('.voucher-detail-page')).toContainText('Depreciation Expense', { timeout: 15000 })
    await expect(page.locator('.voucher-detail-page')).toContainText('Accumulated Depreciation', { timeout: 15000 })
  })
})
