...
const RECORD_ID = 'asset-e2e-1'
const LAYOUT_ID = 'layout-asset-readonly'
const INITIAL_LABEL = 'Asset Name'
const UPDATED_LABEL = 'Asset Name (Updated)'
...
function findFieldLabel(layoutConfig: AnyRecord, fieldCode: string): string {
  ...
  for (const section of sections) {
    ...
    for (const field of directFields) {
      if (field?.fieldCode === fieldCode) return field?.label || ''
    }
    ...
    for (const item of items) {
      const collapseFields = Array.isArray(item?.fields) ? item.fields : []
...
test.describe('Layout Designer Save -> Detail Rendering Regression', () => {
  test('readonly detail page must reflect designer saved label', async ({ page }) => {
    let activeLayoutConfig = buildInitialLayoutConfig()
    let saveCallCount = 0

    await page.addInitScript(() => {
      localStorage.setItem('access_token', 'e2e-designer-token')
    ...
    await page.route('**/*', async (route) => {
      ...
      if (pathname.endsWith(`/api/system/page-layouts/${LAYOUT_ID}/`)) {
        if (route.request().method() === 'PATCH') {
          ...
          if (nextConfig?.sections?.length) {
            activeLayoutConfig = nextConfig
          }
        }

        return fulfillSuccess(route, {
          id: LAYOUT_ID,
          layoutCode: `${OBJECT_CODE}_readonly_e2e`,
      ...
      if (pathname.endsWith(`/api/system/objects/${OBJECT_CODE}/fields/`)) {
        ...
        const activeLabel = findFieldLabel(activeLayoutConfig, 'assetName') || INITIAL_LABEL
        return fulfillSuccess(route, {
          editable_fields: [
            {
              code: 'assetName',
