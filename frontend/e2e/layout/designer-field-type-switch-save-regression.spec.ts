...
const RECORD_ID = 'asset-field-type-1'
const LAYOUT_ID = 'layout-asset-field-type-switch'
...
test.describe('Layout Designer Field Type Switch Save Regression', () => {
  test('field type switch in properties should persist after save and keep detail rendering', async ({ page }) => {
    let activeLayoutConfig = buildInitialLayoutConfig()
    let saveCallCount = 0

    await page.addInitScript(() => {
      localStorage.setItem('access_token', 'e2e-designer-field-type-token')
    ...
    await page.route('**/*', async (route) => {
      ...
      if (pathname.endsWith(`/api/system/page-layouts/${LAYOUT_ID}/`)) {
        ...
        return fulfillSuccess(route, {
          id: LAYOUT_ID,
          layoutCode: `${OBJECT_CODE}_readonly_field_type`,
      ...
      if (pathname.endsWith(`/api/system/objects/${OBJECT_CODE}/runtime/`)) {
        const currentType = String(activeLayoutConfig.sections?.[0]?.fields?.[0]?.fieldType || 'text')
        return fulfillSuccess(route, {
          runtimeVersion: 1,
          fields: {
            editableFields: [
      ...
      if (pathname.endsWith(`/api/system/objects/${OBJECT_CODE}/${RECORD_ID}/`)) {
        return fulfillSuccess(route, {
          ...
          createdAt: '2026-02-26T10:00:00+08:00',
          updatedBy: { username: 'admin' },
          updatedAt: '2026-02-26T10:30:00+08:00'
        })
      }
    ...
    const fieldTypeControl = page.getByTestId('field-prop-fieldType').first()
    ...
    await expect(page.locator('.field-label', { hasText: 'Asset Name' }).first()).toBeVisible()
