...
async function mockApis(page: Page) {
  await page.addInitScript(() => {
    localStorage.setItem('access_token', 'e2e-object-layout-entry-token')
    localStorage.setItem('current_org_id', 'org-object-layout-entry')
  ...
  await page.route('**/*', async (route) => {
    ...
    if (pathname.endsWith('/api/system/objects/User/me/')) {
      return fulfillSuccess(route, {
        id: 'user-object-layout-entry',
        username: 'admin',
        roles: ['admin'],
        permissions: ['*'],
        primaryOrganization: { id: 'org-object-layout-entry', name: 'Regression Org', code: 'REG' }
      })
    }
...
test.describe('Object Layout Entry Navigation', () => {
  ...
  test('should navigate from object list Layouts button to layout page with consistent object title', async ({ page }) => {
    ...
    for (const entry of OBJECT_CASES) {
      await gotoAppRoute(page, `/objects/${entry.code}`)
      await expect(page.locator('main')).toContainText(entry.expectedTitle, { timeout: 30_000 })
