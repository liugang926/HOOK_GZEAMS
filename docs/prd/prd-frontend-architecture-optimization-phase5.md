...
2.  **Strict Focus Trapping:** Implement a `v-focus-trap` directive (or integrate an existing robust library like `focus-trap-vue`). Apply this to all side-drawers (e.g., `MaintenanceList.vue`, `ITAssetList.vue` drawers) to ensure the `Tab` key cycles only within the drawer content.
3.  **Deterministic `tabindex` flows:** For high-volume data entry forms (e.g., `AssetReceiptDetail.vue`, `VoucherList.vue` forms), explicitly sequence the `tabindex` to follow the visual layout (Left-to-Right, Top-to-Bottom), overriding default browser DOM order if necessary to match the semantic grid.
